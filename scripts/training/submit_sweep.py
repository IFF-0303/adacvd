# this script creates a sweep of training runs based on the settings in the train_settings.yaml file.
# It creates a directory for each run and copies the train_settings.yaml file into it.
# It also creates a submission file for each run and submits it to condor.
...
if __name__ == "__main__":
    ...
    if not args.sweep_settings:
        ...
        for i, settings in enumerate(combination_dicts):
            ...
            fgs = new_settings["data"]["feature_config"]["feature_groups"]
            if (
                "blood_samples" in fgs
                or "selected_features_top_100" in fgs
                or len(fgs) > 4
            ):
                print(fgs)
                # new_settings["training"]["batch_size"] = 8
                new_settings["training"]["eval_batch_size"] = 2
                new_settings["condor"]["request_gpus"] = 4
                new_settings["training"]["batch_size"] = 8
                print("Increasing number of GPUs for larger feature groups.")
            ...
            #     n_full = 387841 / 12
            #     num_epochs = max(int(n_full / num_training_samples), 1)
            #     print(
            ...
            # create submission file, and submit condor job
            port = random.choice(ports)
            condor_arguments = f"launch --main_process_port {port} pandora/training/train_model.py --train_dir={train_dir} --device=cuda"
            submission_file = "submission_file.sub"
